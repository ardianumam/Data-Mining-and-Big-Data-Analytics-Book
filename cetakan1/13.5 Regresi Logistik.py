...
def outputSigmoid(W, inputDataset):
    ...
    for i in range(0,inputDataset.shape[0]):
        ...
        hasilSigmoid = np.concatenate((hasilSigmoid, fungsiSigmoid), axis=0)
...
def gradDescent(initialWeight, inputDataset, label, step, iteration, tolerance):
    ...
    for i in range(0, iteration):
        ...
        w_baru = w_lama - step * np.transpose(designMat).dot(np.subtract(outSigm,label))
...
w = gradDescent(initial_W, inputTraining, labelTraining, konstantaStep, banyaknyaIterasi, tolerance)

#testing: masukkan nilai W dan input x-nya ke fungsi sigmoid kembali
#kemudian bulatkan (agar <0,5 masuk ke kelas 1 (label=0) dan >0,5 masuk ke kelas 2 (label=1))
