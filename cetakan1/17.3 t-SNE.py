...
def tsne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0, max_iter=()):
    """
...
if __name__ == "__main__":
    print("Jalankan Y = tsne.tsne(X, no_dims, perplexity) untuk melakukan t-SNE.")
    ...
    Y,P,Q = tsne(X, no_dimensi, dimensi_reduksi_dg_PCA, perplexityValue, max_iter)
    ...
    nQ, binsQ, patchesQ = pylab.hist(Q2500, color="green", alpha=0.8,
                                     label="Q" + ",mean: " + str(np.mean(Q)) + ",var: " + str(np.var(Q)))
    nP, binsP, patchesP = pylab.hist(P2500, color="red", alpha=0.8,
                                     label="P" + ",mean: " + str(np.mean(P)) + ",var: " + str(np.var(P)))
    ...
    for label in np.unique(labels):
        pylab.scatter(Y[labels == label, 0], Y[labels == label, 1], label=label, s=30, marker=marker.__next__())
    pylab.legend()
    pylab.show()
