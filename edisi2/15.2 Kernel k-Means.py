...
initMethod = "random" #options = random, byCenterDistance,
                                #byOriginDistance
...
def initCluster(dataInput, nCluster, method):
    ...
    if (method == "byCenterDistance"):
        ...
        dataNew = np.array(np.concatenate((euclideanMatrix,
                                           dataInput), axis=1))
        dataNew = dataNew[np.argsort(dataNew[:, 0])]
        dataNew = np.delete(dataNew, 0, 1)
        divider = dataInput.shape[0]/nCluster
        for i in range(0, dataInput.shape[0]):
            listClusterMember[np.int(
                np.floor(i/divider))].append(dataNew[i,:])
...
def thirdTerm(memberCluster):
    ...
    for i in range(0, memberCluster.shape[0]):
        for j in range(0, memberCluster.shape[0]):
            result = result + RbfKernel(memberCluster[i, :],
                                        memberCluster[j, :], std)
...
def kMeansKernel(data, initMethod):
    ...
    while(True):
        ...
        for i in range(0, nCluster):
            ...
            centroid = np.concatenate((centroid,
        ...
        kernelResultClusterAllCluster = np.ndarray(
            shape=(data.shape[0], 0))
        #tempatkan data ke klaster dengan
        #nilai jarak euclidean kernelnya plg kecil
        for i in range(0, nCluster):#repeat for all cluster
            ...
            for j in range(0, data.shape[0]): #ulangi untuk semua data
                term2 = secondTerm(data[j,:],
                                   np.asmatrix(memberInit[i]))
                matrixTerm2 = np.concatenate((matrixTerm2,
                                              term2), axis=0)
            matrixTerm2 = np.asmatrix(matrixTerm2)
            kernelResultClusterI = np.add(-1*matrixTerm2,
                                          matrixTerm3)
            kernelResultClusterAllCluster = np.concatenate((
                kernelResultClusterAllCluster,
                kernelResultClusterI), axis=1)
        clusterMatrix = np.ravel(np.argmin(np.matrix(
            kernelResultClusterAllCluster), axis=1))
        listClusterMember = [[] for l in range(k)]
        #menempatkan data berdasarkan klaster yg didapat
        for i in range(0, data.shape[0]):
            listClusterMember[np.asscalar(
                clusterMatrix[i])].append(data[i,:])
        for i in range(0, nCluster):
            print("Anggota dari klaster ke-", i, ": ",
                  listClusterMember[0].__len__())
...
plotResult(clusterResult, centroid,
           str(iterationCounter) + ' (konvergen)', 1)